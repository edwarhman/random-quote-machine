...
export const fetchIndex = createAsyncThunk(
  ...
  async () => {
    const response = await fetch(
      `https://api.biblia.com/v1/bible/contents/RVR60?key=${API_KEY}`
    );
...
export const fetchQuote = createAsyncThunk(
  ...
  async (verseInfo) => {
    const response = await fetch(
      `https://api.biblia.com/v1/bible/content/RVR60.text.json?passage=${
        verseInfo.passage + verseInfo.chapter
      }&style=oneVersePerLineFullReference&key=${API_KEY}`
    );
    return await response.json();
  }
);
